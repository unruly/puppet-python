...
define python::virtualenv (
  ...
  if $ensure == 'present' {
    ...
    $distribute_pkg = $distribute ? {
      true     => 'distribute==0.6.49',
      default  => 'setuptools==33.1.1',
    }
    $pypi_index = $index ? {
      false   => '',
      default => "-i ${index}",
    ...
    exec { "python_virtualenv_${venv_dir}":
      command     => "true ${proxy_command} && ${virtualenv_cmd} ${system_pkgs_flag} -p ${python} ${venv_dir} && ${pip_cmd} wheel --help > /dev/null 2>&1 && { ${pip_cmd} wheel --version > /dev/null 2>&1 || wheel_support_flag='--no-binary :all:'; } ; { ${pip_cmd} --log ${venv_dir}/pip.log install ${pypi_index} ${proxy_flag} \$wheel_support_flag pip ${distribute_pkg} || ${pip_cmd} --log ${venv_dir}/pip.log install ${pypi_index} ${proxy_flag} pip ${distribute_pkg} ;}",
      user        => $owner,
